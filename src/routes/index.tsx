...
function Home() {
  ...
  const definitionCount = subjects.reduce(
    (sum, subject) => sum + subject.chapters.reduce((n, chapter) => n + (chapter.definitions?.length ?? 0), 0),
    0,
  );
  const formulaCount = subjects.reduce(
    (sum, subject) => sum + subject.chapters.reduce((n, chapter) => n + chapter.formulas.length, 0),
    0,
  );
    ...
    <div>
      ...
      <section className="mx-auto max-w-6xl px-5">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Organized CBSE Class 12 revision resources for Physics, Chemistry, Mathematics and Computer Science.
        </p>
      ...
      <section className="mx-auto mt-24 max-w-6xl px-5">
        <div className="glass neon-ring grid gap-6 rounded-3xl p-8 sm:grid-cols-3">
          {[
            { k: String(chapterCount), v: "chapters covered" },
            { k: String(definitionCount), v: "definitions" },
            { k: String(formulaCount), v: "formulas" },
          ].map((item) => (
            <div key={item.v}>
